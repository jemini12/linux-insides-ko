커널 초기화(Kernel initialization). Part 1.
================================================================================

커널 코드의 첫번째 단계
--------------------------------------------------------------------------------
이전 [글](https://0xax.gitbooks.io/linux-insides/content/Booting/linux-bootstrap-6.html)에서 다룬 내용은 리눅스 커널의 [부팅 과정](https://0xax.gitbooks.io/linux-insides/content/Booting/index.html) 이었다면 이번 챕터에서는 리눅스 커널의 초기화 과정(Linux Kernel Initialization)에 대해서자세하게 이야기 해보겠다. 리눅스 커널의 이미지가 메모리상의 적절한 곳에 해제(decompressed)되어 배치된 이후에는 커널이 동작하기 시작한다. 지금까지 다뤘던 내용은 전부 리눅스 커널 코드의 가장 첫번째 부분이 실행되기 위해서 준비하는 과정에 관한 것이었다. 이제 우리는 커널 내부로 들어와서 커널의 초기화 과정의 시작부터 [pid](https://en.wikipedia.org/wiki/Process_identifier) `1`을 실행하기 직전까지의 과정을 다루는데 집중할 것이다. 커널은 첫번째 프로세스인 `init`을 실행하기 전에 해야할 것들이 많다. 커널이 준비하는 모든것들을 전부다루길 희망하면서 이 긴 챕터를 시작해본다.

첫번째로 우리는 커널의 엔트리 포인트 부터 다룰 것이다. 엔트리 포인트는 [arch/x86/kernel/head_64.S](https://github.com/torvalds/linux/blob/master/arch/x86/kernel/head_64.S) 에 존재하는데 우리는 이 부분부터 점차 더 깊은 부분까지 다룰 예정이다. 그리고 우리는 커널의 첫번째 준비과정 중 하나인 페이지 테이블의 첫번째 초기화 부터 커널 공간안의 새로운 디스크립터까지 [init/main.c](https://github.com/torvalds/linux/blob/master/init/main.c) 안에 있는 `start_kernel` 함수가 실행되기 전에 존재하는 많은 것들을 다룰 것이다. 

이전 [챕터](https://0xax.gitbooks.io/linux-insides/content/Booting/index.html)의 마지막 [부분](https://0xax.gitbooks.io/linux-insides/content/Booting/linux-bootstrap-6.html)은 [arch/x86/boot/compressed/head_64.S](https://github.com/torvalds/linux/blob/master/arch/x86/boot/compressed/head_64.S) 어셈블리 코드의 jmp 지시자 였다.
```assembly
jmp	*%rax
```
이 시점에 `rax` 레지스터는 리눅스 커널의 엔트리 포인트 주소를 가지고 있는데 해당 주소는 [arch/x86/boot/compressed/misc.c](https://github.com/torvalds/linux/blob/master/arch/x86/boot/compressed/misc.c) 소스 코드 파일에 존재하는 `decompress_kernel` 함수의 호출을 통해서 얻은 것이다. 이로 인해 커널 setup code에 있는 마지막 지시자는 리눅스 커널의 엔트리 포인트의 주소로 이동는 것이 된다. 우리는 이미 리눅스 커널의 엔트리 포인트가 정의되어 있는지 알기 때문에 리눅스 커널이 실행된 이후에 어떤 것들을 하는지 확인할 수 있다.
